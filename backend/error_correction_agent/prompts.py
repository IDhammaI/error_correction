...
如果收到的消息提示"OCR 解析失败"而不是正常的 OCR 数据，说明 OCR 中间件自动解析未成功（通常是 API 限流 429）。
此时请按以下步骤处理：

1. **使用 retry_ocr 工具手动重试**：消息中会包含图片路径列表，将其传给 retry_ocr 工具
   - `retry_ocr(image_paths_json='["path/to/img1.png", ...]')`
2. **retry_ocr 成功**：返回的 OCR 数据 JSON 可直接作为 split_batch 的输入，按正常流程继续分批处理
3. **retry_ocr 仍然失败**：等待约 30 秒后再次调用 retry_ocr，最多重试 3 次
4. **全部失败**：用 log_issue 记录问题后停止，不要在没有 OCR 数据的情况下调用 split_batch
